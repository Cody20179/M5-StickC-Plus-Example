...
**4 SOUND** — dBFS 音量表、主頻率、**8 段八度頻譜柱狀圖**（31 Hz ~ 4 kHz）

![SOUND](docs/page4-sound.png)

> ⚠ **已知問題**：上圖中柱狀圖的刻度沒調好，安靜環境下就全部滿格。
> 頻段 dB 的映射範圍需要修正，見 [Issues](#已知問題)。
...
這些是把螢幕擷取出來、實際看到畫面之後才發現的 —— 光看序列埠的數字不會注意到。

### `multiclock` SOUND 頁的柱狀圖刻度錯誤

柱高用 `(bandDb + 20) / 70` 映射到 0~1。但那些是**相對於 int16 計數的 FFT 幅值 dB**，
不是 dBFS：安靜環境實測各頻段落在 39~60 dB，滿刻度正弦波則接近 132 dB。

所以 `(60+20)/70 = 1.14` 會被箝到 1.0 —— **安靜時就已經全部滿格**，整頁失去意義。
合理的映射大約是 `(bandDb - 40) / 70`。

### SOUND 頁的頻段標籤與底部提示重疊

`2k` / `4k` 兩個標籤畫在 y=121，和底部的 `BtnB reset pk` 撞在一起。

### ENV 頁的 `pk` 與累計值重疊

振動峰值 `pk` 靠左畫在 x=118，累計量那串是右對齊，兩者在同一行相撞，
畫面上變成 `pksum 0k mg-s`。

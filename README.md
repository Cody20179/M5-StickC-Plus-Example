...
| 08 | [`alarm`](08-alarm) | 定時提醒：每 N 分鐘蜂鳴器響 + LED 閃 |
...
### 10. 序列埠讀取數字參數時不能用 `read()` 判斷結束

`Serial.read()` 會把字元消耗掉，所以拿它來偵測「參數結束」等於**把下一個指令的
第一個字元吃掉**。改用 `Serial.peek()`，不屬於這個參數的字元就留在緩衝區。

實測踩過：連續送 `I5` 再送 `T2026...`，`I` 的讀取器在等第二位數字時吃掉了那個 `T`，
後面整串日期被主迴圈當成一堆無效指令丟棄 —— **對時靜默失敗，沒有任何錯誤訊息**。

### 11. FFT 幅值的 dB 不是 dBFS
